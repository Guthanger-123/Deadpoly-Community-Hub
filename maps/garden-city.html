...
<body>
  ...
  <script>
    // If your image isn’t 1264x1264, update these two numbers to match the PNG’s real size.
    const IMAGE_WIDTH = 1264;
    const IMAGE_HEIGHT = 1264;

    const IMAGE_URL = "../GCMap_.png";

    const map = L.map('map', {
      crs: L.CRS.Simple,
      minZoom: -6,   // allow zooming out
      maxZoom: 6,
      zoomSnap: 0.25,
      zoomDelta: 0.25
    });
    ...
    // show whole image first
    map.fitBounds(bounds);

    // keep panning roughly constrained to image
